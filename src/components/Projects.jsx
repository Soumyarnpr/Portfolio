		...
		<section id="projects" className="section-shell scroll-mt-28 py-20">
			...
			<div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
				{projects.map((project) => (
					<article
						key={project.title}
						className="project-card reveal group relative min-h-65.5 overflow-hidden rounded-lg bg-rich-black/70 shadow-2xl shadow-black/30"
					>
						<Image
							src={project.image}
						...
						<div className="absolute inset-x-0 bottom-0 z-2 p-5">
							<div className="mb-4 flex flex-wrap gap-2">
								{project.tags.map((tag) => (
									<span
										key={tag}
										className="glass-chip px-3 py-1 text-xs font-bold text-baby-powder/86"
									>
										{tag}
									</span>
								))}
							</div>
							<h3 className="text-2xl font-extrabold text-imperial-red">
								{project.title}
							</h3>
							...
							<div className="mt-5 flex min-h-10 items-center gap-3">
								{project.links.length > 0 ? (
									...
									<span className="text-sm font-semibold text-baby-powder/56">
										Static build showcase
									</span>
								)}
							</div>
						</div>
